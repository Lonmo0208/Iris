...
public class ShadowCompositeRenderer {
	...
	private static void setupMipmapping(net.irisshaders.iris.targets.RenderTarget target, boolean readFromAlt) {
		...
		IrisRenderSystem.texParameteri(texture, GL46C.GL_TEXTURE_2D, GL46C.GL_TEXTURE_MIN_FILTER, target.getInternalFormat().getPixelFormat().isInteger() ? GL46C.GL_NEAREST_MIPMAP_NEAREST : GL46C.GL_LINEAR_MIPMAP_LINEAR);
	}
	...
	private static void resetRenderTarget(RenderTarget target) {
		...
		int filter = GL46C.GL_LINEAR;
		if (target.getInternalFormat().getPixelFormat().isInteger()) {
			filter = GL46C.GL_NEAREST;
		}

		IrisRenderSystem.texParameteri(target.getMainTexture(), GL46C.GL_TEXTURE_2D, GL46C.GL_TEXTURE_MIN_FILTER, filter);
		IrisRenderSystem.texParameteri(target.getAltTexture(), GL46C.GL_TEXTURE_2D, GL46C.GL_TEXTURE_MIN_FILTER, filter);
	}
