...
public class IrisCycleButton<T> extends AbstractButton {
	...
	protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
		...
		GuiUtil.drawButton(graphics,this.getX(), this.getY(), this.getWidth(), this.getHeight(), this.isHoveredOrFocused(), !this.isActive());
	...
	private void renderString(@UnknownNullability GuiGraphicsExtractor guiGraphics, Font font, int color) {
		Component message = this.getMessage();
		int x = this.getX() + this.getWidth() / 2;
		int y = this.getY() + (this.getHeight() - 8) / 2;

		guiGraphics.centeredText(font, message, x, y, color | Mth.ceil(this.alphaSupplier.getAsFloat() * 255.0F) << 24);
	}
