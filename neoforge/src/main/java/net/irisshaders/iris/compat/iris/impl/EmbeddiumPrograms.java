...
public class EmbeddiumPrograms {
	...
	public EmbeddiumPrograms(IrisRenderingPipeline pipeline, ProgramSet programSet, ProgramFallbackResolver resolver,
		...
		net.irisshaders.iris.platform.IrisForgeMod.LOGGER.info("Transforming Embeddium shaders...");
		...
		stopwatch.stop();
		net.irisshaders.iris.platform.IrisForgeMod.LOGGER.info("Transforming Embeddium shaders completed in {}", stopwatch);
...
//			programSet.getPackDirectives().getTextureMap());
		...
		Map<PatchShaderType, String> transformedNew = ShaderTransformer.transform(
				...
				source.getTessEvalSource().orElse(null),
				source.getFragmentSource().orElse(null),
				alphaTest, net.irisshaders.iris.platform.IrisForgeMod.getVertexFormat(),
				programSet.getPackDirectives().getTextureMap());
