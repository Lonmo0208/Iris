...
import net.irisshaders.iris.compat.embeddium.ShaderTypeUtil;
...
import net.irisshaders.iris.vertices.embeddium.terrain.FormatAnalyzer;
...
public class EmbeddiumPrograms {
	...
	private Map<PatchShaderType, GlShader> createGlShaders(String passName, Map<PatchShaderType, String> transformed) {
		...
		for (Map.Entry<PatchShaderType, String> entry : transformed.entrySet()) {
			...
			newMap.put(entry.getKey(), new GlShader(ShaderTypeUtil.fromGlShaderType(entry.getKey().glShaderType.id),
