...
import net.irisshaders.iris.compat.embeddium.impl.BindUtil;
import net.irisshaders.iris.compat.embeddium.impl.shader_overrides.GlUniformFloat2v;
...
public class EmbeddiumShader implements ChunkShaderInterface {
	...
	public EmbeddiumShader(IrisRenderingPipeline pipeline, EmbeddiumPrograms.Pass pass, ShaderBindingContext context,
        ...
        this.uniformModelViewMatrix = BindUtil.bindUniformOptional(context,"iris_ModelViewMatrix", GlUniformMatrix4f::new);
		this.uniformModelViewMatrixInv =BindUtil.bindUniformOptional(context,"iris_ModelViewMatrixInverse", GlUniformMatrix4f::new);
		this.uniformNormalMatrix = BindUtil.bindUniformOptional(context,"iris_NormalMatrix", GlUniformMatrix3f::new);
		this.uniformProjectionMatrix = BindUtil.bindUniformOptional(context,"iris_ProjectionMatrix", GlUniformMatrix4f::new);
		this.uniformProjectionMatrixInv = BindUtil.bindUniformOptional(context,"iris_ProjectionMatrixInv", GlUniformMatrix4f::new);
		this.uniformRegionOffset = BindUtil.bindUniformOptional(context,"u_RegionOffset", GlUniformFloat3v::new);
		this.uniformTexCoordShrink = BindUtil.bindUniformOptional(context,"u_TexCoordShrink", GlUniformFloat2v::new);
	...
	public void setupState() {
		...
		this.uniformTexCoordShrink.set(
				(float) (subTexelOffset + ((1.0D / textureAtlas.callGetWidth()) / subTexelPrecision)),
				(float) (subTexelOffset + ((1.0D / textureAtlas.callGetHeight()) / subTexelPrecision))
		);
